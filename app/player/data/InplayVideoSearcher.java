...
public class InplayVideoSearcher {
	...
	public static LinkedHashSet<InplayVideoDetailsDTO> getListForSearchText(String text) {
		...
		for(InplayVideoDetailsDTO dto : videoSet) {
			if(resultCounter==5) return result;
			String videoTitle = dto.getVideoTitle();
			if(videoTitle==null || videoTitle.trim().length()==0) continue;
	...
	public static Set<InplayVideoDetailsDTO> getGenereVideoList(String genere) {
		Map<String, TreeSet<InplayVideoDetailsDTO>> videoMap = InplayDataProvider.getVideoMap();
